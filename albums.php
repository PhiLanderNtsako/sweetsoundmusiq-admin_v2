                    ...
                    <div class="data-table-list">
                        ...
                        <div class="table-responsive">
                            <table id="data-table-basic" class="table table-striped">
                                ...
                                <tbody>
                                <?php
                                    $query_musiq = "SELECT * FROM ((artist INNER JOIN  musiq ON artist.artist_id = musiq.artist_id) INNER JOIN album ON musiq.musiq_id = album.musiq_id) WHERE musiq.musiq_type = 'Album'";
                                    $result_set = mysqli_query($conn, $query_musiq);
                                    // $row=mysqli_fetch_assoc($result_set);

                                    // $sel_musiq = "SELECT * FROM musiq, artist WHERE musiq.artist_id = artist.artist_id AND musiq.musiq_type = 'Album' ORDER BY musiq_id DESC";
                                ...
                                ?>
                                        <tr>
                                            ...
                                            <td>
                                            <?php
                                                if(NULL == $row['songs_added']){
                                            ?>
                                                    <a href="add-album-songs.php?<?php echo 'artist='.$row['artist_id'].'&album='.$row['album_id'].'&musiq='.$row['musiq_id'].'&numberofsongs='.$row['number_of_songs']?>" class="btn btn-success notika-btn-success"> Add Songs</a><br><br>
                                            <?php       
                                                }elseif(NULL == $row['links_added']){
                                            ?>      
                                            ...
                                            <?php
                                                }elseif($row['active_yn']){
                                            ?>
                                                    <button class="btn btn-success notika-btn-success" disable>Active</button><br><br>

                                                    <a href="view-album-songs.php?<?php echo 'artist='.$row['artist_id'].'&album='.$row['album_id'].'&musiq='.$row['musiq_id'].'&numberofsongs='.$row['number_of_songs']?>" class="btn btn-success notika-btn-warning"> View Album</a>
                                            <?php
                                                }else{
                                            ?>
